...
- When changing CLI commands/flags, API fields, or product behavior documented by built-in skills under `server/internal/service/builtin_skills/*`, update the relevant `references/<domain>.md` in the same PR. Shipped skill payload carries no source references — no repository paths, source file names, migration ordinals or Go test names; a test enforces this, and the evidence for a contract belongs in `builtin_skills_test.go`.
