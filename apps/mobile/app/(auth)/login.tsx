...
export default function Login() {
  ...
  const loadConfig = useCallback(async () => {
    ...
    try {
      ...
      setConfigError(mapAuthError(err, t("login.config.load_error")));
    }
  }, [t]);
  ...
  useEffect(() => {
    ...
    if (response.type !== "success") {
      if (response.type !== "dismiss" && response.type !== "cancel") {
        setError(t("login.sso.not_completed"));
      }
      setSubmitting(false);
      return;
    }
    ...
    if (!code || !verifier) {
      setError(t("login.sso.callback_incomplete"));
      setSubmitting(false);
      return;
    }
    void loginWithSSO(code, verifier, redirectUri)
      ...
      .catch((err) => {
        ...
        setError(mapAuthError(err, t("login.sso.complete_error")));
        setSubmitting(false);
      });
  }, [loginWithSSO, request, response, t, useSySso]);
  ...
  const signInWithSSO = () => {
    ...
    void promptAsync().catch((err) => {
      setError(mapAuthError(err, t("login.sso.open_error")));
      setSubmitting(false);
    });
  };
      ...
      <SafeAreaView className="flex-1 bg-background">
        <View className="flex-1 justify-center px-6 gap-4 items-center">
          ...
          <Text className="text-lg font-semibold text-foreground">
            {configError
              ? t("login.config.error_title")
              : t("login.config.loading_title")}
          </Text>
          {configError ? (
            <>
              ...
              <Button size="lg" onPress={() => void loadConfig()}>
                <Text>{t("login.config.retry")}</Text>
              </Button>
            </>
          ) : null}
        </View>
      ...
      <SafeAreaView className="flex-1 bg-background">
        ...
        >
          <View className="flex-1 justify-center px-6 gap-6">
            ...
            >
              <Text>
                {submitting ? t("login.sending") : t("login.send_code")}
              </Text>
            </Button>
          </View>
        </KeyboardAvoidingView>
  ...
  return (
    <SafeAreaView className="flex-1 bg-background">
      <View className="flex-1 justify-center px-6 gap-6">
        ...
        >
          <Text>
            {submitting ? t("login.sso.signing_in") : t("login.sso.continue")}
          </Text>
        </Button>
      </View>
    </SafeAreaView>
