  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-muted/30 px-4 py-10">
      <div className="w-full max-w-md rounded-xl border bg-background p-6 shadow-sm">
        <div className="text-sm font-semibold">授权 Figma 插件</div>
        <p className="mt-2 text-sm text-muted-foreground">允许 Figma 插件将 Gallery Native 设计稿上传到一个 Multica 工作区。</p>

        {!sessionId ? <div className="mt-4 rounded-md border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive">缺少授权会话。</div> : null}
        {error ? <div className="mt-4 rounded-md border p-3 text-sm text-muted-foreground">请先登录 Multica，然后重新打开此授权链接。</div> : null}

        <label className="mt-5 block text-xs font-medium text-muted-foreground">工作区</label>
        <select
        ...
        {authorize.isSuccess ? (
          <div className="mt-5 rounded-md border border-emerald-500/30 bg-emerald-500/10 p-3 text-sm text-emerald-700 dark:text-emerald-300">已授权。请返回 Figma，插件会自动完成登录。</div>
        ) : (
