...
func runReasonixEffortTurn(t *testing.T, sessionResult, level string) (Result, string) {
	...
	// Upstream hands the stub its log path through the inherited test-process
	// environment, but this fork's privacy gate strips inherited env down to
	// an allowlist before spawning agents (agentguard.AllowedInheritedEnvKey).
	// Explicit Config.Env entries are appended after the filter by design, so
	// the side channel moves there instead of widening the allowlist.
	backend, err := New("reasonix", Config{
		ExecutablePath: fakePath,
		Env:            map[string]string{"REASONIX_RPC_LOG": logPath},
