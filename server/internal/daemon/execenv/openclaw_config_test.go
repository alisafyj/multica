...
func TestPrepareOpenclawConfigFailsClosedOnResolvedConfigError(t *testing.T) {
	...
	// No CLI diagnostic here: this fork resolves the active config by READING
	// the file, because OpenClaw 2026.7.1 rejects a pathless
	// `openclaw config get --json` (see TestPrepareOpenclawConfigConfigGetPathRequired).
	// Failing closed on the unparseable file is the behaviour under test.
	if !strings.Contains(err.Error(), "openclaw active config") {
		t.Errorf("error %q does not name the unparseable active config", err.Error())
	}
	if strings.Contains(err.Error(), "must-not-leak") {
		t.Errorf("error leaked a non-diagnostic JSON field: %q", err.Error())
	}
