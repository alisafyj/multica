...
func (d *Daemon) Run(ctx context.Context) error {
	...
	// without the full Run setup. The initial workspace request also returns
	// the authoritative expiry for non-renewable SSO/service credentials.
...
func (d *Daemon) reportAuthenticationExpired(taskID string, result TaskResult, taskLog *slog.Logger) {
	...
	if err := d.client.FailTask(reportCtx, taskID, errAuthenticationExpired.Error(), result.SessionID, result.WorkDir, taskfailure.ReasonAuthenticationExpired.String(), result.SessionRolloutMissing); err != nil {
