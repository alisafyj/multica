...
func buildDesignRestorePrompt(task Task) string {
	...
	b.WriteString("- For production restore plans (`restore_plan.repo.mode == \"production_candidate\"`), write only under `restore_plan.targets.selected.path` and `restore_plan.execution.allowedPaths`; do not write prototype HTML or files under `fengchenDoc/gallery-native-agent-test`.\n")
	b.WriteString("- If a production restore plan has no `targets.selected.path`, stop and return blocked instead of guessing a path.\n")
