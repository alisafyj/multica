...
func TestPreflightAuth_RenewsBeforeWorkspaceSyncOnExpiredToken(t *testing.T) {
	...
	mu.Lock()
	defer mu.Unlock()
	if len(seen) < 2 || seen[0] != "/api/tokens/current/renew" || seen[1] != "/api/workspaces" {
		t.Fatalf("unexpected preflight order: %v", seen)
	}
	if !strings.Contains(buf.String(), "multica login") {
		t.Fatalf("missing re-login warning: %s", buf.String())
	}
}
...
func TestPreflightAuth_SyncProceedsWhenRenewIsNoOp(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/tokens/current/renew":
			_ = json.NewEncoder(w).Encode(map[string]any{"expires_at": "2099-01-02T03:04:05Z", "renewed": false})
		case "/api/workspaces":
