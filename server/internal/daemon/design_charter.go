...
func designerCharter() string {
	...
	b.WriteString("- Route the design's tunable decisions through named custom properties on `:root` and read them everywhere: `--accent` (the action / brand colour), `--scale` (type scale multiplier, 1 = as designed), `--density` (spacing multiplier, 1 = as designed) and, where the design animates, `--motion` (0 disables transitions, 1 = as designed). Writing `var(--accent)` instead of a literal costs nothing while you are writing the CSS and cannot be added afterwards without rewriting the stylesheet, which is why it is not optional. It also lets the workspace retune a delivered design without spending another run on it.\n")
