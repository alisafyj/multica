...
| A round launched by a `test_run` autopilot carries the autopilot run's attribution on its case tasks and closes that run when it converges, aborts or is parked blocked | `server/internal/handler/autopilot_test_run.go` (`autopilotTestRunLauncher`, `settleAutopilotTestRun`), `server/internal/service/autopilot_test_run.go` (`SettleAutopilotTestRun`) |
