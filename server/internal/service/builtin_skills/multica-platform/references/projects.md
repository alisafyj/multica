...
- `document` — external specification or business-rule page, with
  `resource_ref.url` (http/https only), required `title`, and optional `summary`
  (≤500 chars). Agents fetch and read the URL before implementing or testing
...
3. Check `github_repo.resource_ref.url`, optional `ref`, `default_branch_hint`;
   `local_directory.resource_ref.daemon_id`; and `document.resource_ref.url` +
   `title`.
