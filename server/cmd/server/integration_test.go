...
func TestRouterAuthMode(t *testing.T) {
	...
	assertStatus := func(name, baseURL, method, path string, want int, bearerToken ...string) {
		t.Helper()
	...
	for _, route := range ssoRoutes {
		if strings.Contains(route.path, "service-account") {
			assertStatus("SSO route absent in legacy", testServer.URL, route.method, route.path, http.StatusNotFound, testToken)
		} else {
