...
Text overflow uses measured text width and treats both ellipsis and wrapping as valid only when compiler-owned expectations agree with emitted metadata and the text remains within its paint bounds; wrapping also requires enough available height. Overlap compares all relevant visible generated/component bounds across business regions, skips true ancestor/descendant containment, and permits overlap only when both compiler-owned expectations declare the same non-empty compatible overlay role. Off-frame checks generated bounds against frame or horizontal-scroll content bounds. Count consistency compares semantic roles to manifest. Residue compares normalized business-region text against pre-clear template text, excluding PageSpec text and shell allowlist descendants. Pagination must start below the last row. Generated component conformance compares the actual canonical output subtree, including resolved primitive styles, geometry, assets, and bindings, with compiler-owned manifest evidence rather than trusting layer metadata.
