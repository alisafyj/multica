...
M0、M1、M2 已于 2026-09-06 完成第一版（M2 剩余：测试机开关、并行数设置、Web 端实时画面，并入 M4）；M3（页面、扫码 / 粘贴配对、无障碍通道与全部动作、ADB 键盘、保持亮屏、停止与审批）与 M4（中枢限流 / owner / 审计查询 / 租约标签 / IME 切换、按轮次并行数“完成一条放行一条”、测试机开关、runtime 页设备中枢卡片与配对二维码、Web 端实时画面）于 2026-09-07 完成第一版；剩余：真机端到端验收（APK 已装到荣耀 PTP-AN10，待登录后配对）、守护进程托管中枢子进程。
...
M0—M4 第一版已完成（2026-09-06 / 09-07）：M0 + M2 已随 PR #81 合入 main；M3 + M4 在分支 `feat/mobile-device-executor`（PR #82，待合并）；M1 在仓库 `coder-zkl1988/multica-device-mcp`（M4 的中枢改动已在其 main）。端到端验收状态：中枢已在本机识别 5 台小米 + 1 台荣耀真机（adb 轨道）；带设备执行器与 ADB 键盘的 APK 已装到荣耀 PTP-AN10（`ai.multica.mobile.dev`，API 指向 fork 的 staging），还差：在手机上登录、扫码配对、在 runtime 页打开“测试机”、派发一条 `android_device` 用例看无障碍轨与实时画面。小米（HyperOS）拦截了 USB 安装，需要在手机的“USB 安装管理”里放行后重装。下一步：这次真机验收，然后 M5 需求闭环。
