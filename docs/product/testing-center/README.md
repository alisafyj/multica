...
M0—M5 第一版已完成（2026-09-06 / 09-07）：M0 + M2 已随 PR #81 合入 main；M3 + M4 在 PR #82（分支 `feat/mobile-device-executor`）；M5 在 PR #83（分支 `feat/testing-m5-requirement-loop`，基于 #82，合并后自动指向 main）；M1 在仓库 `coder-zkl1988/multica-device-mcp`。端到端验收状态不变：中枢已识别 5 台小米 + 1 台荣耀（adb 轨道），荣耀已装带执行器与 ADB 键盘的 APK（`ai.multica.mobile.dev`，API 指向 fork 的 staging），还差登录、配对、打开“测试机”、派发一条 `android_device` 用例。下一步：真机验收；M6 扩展（无障碍树工具已随 M3 落地，剩 iOS via Mac、autopilot 回归、按差异推荐回归）。
