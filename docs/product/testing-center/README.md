...
M0—M5 第一版已完成（2026-09-06 / 09-07）：M0 + M2 已随 PR #81 合入 main；M3 + M4 在 PR #82（分支 `feat/mobile-device-executor`）；M5 在 PR #84（分支 `feat/testing-m5-requirement-loop`，基于 #82，合并后自动指向 main）；M1 在仓库 `coder-zkl1988/multica-device-mcp`。端到端验收状态不变：中枢已识别 5 台小米 + 1 台荣耀（adb 轨道），荣耀已装带执行器与 ADB 键盘的 APK（`ai.multica.mobile.dev`，API 指向 fork 的 staging），还差登录、配对、打开“测试机”、派发一条 `android_device` 用例。M6 在 PR #85（分支 `feat/testing-m6-extensions`，基于 #84）：iOS 经 PulsePhone 接入（中枢 e694df0）、`testcase recommend` 与推荐对话框、autopilot `test_run` 模式（迁移 915/916，下一个 fork 前缀 917）。下一步：真机验收（Android 经荣耀；iPhone 插到装了 PulsePhone 的测试机即可被中枢发现，本会话无 iPhone 可试）；三条 PR 合并顺序 #82 → #84 → #85。
