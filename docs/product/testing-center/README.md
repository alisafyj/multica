...
M0—M6 第一版已完成（2026-09-06 / 09-07）：M0 + M2 已随 PR #81 合入 main；PR #82 在只含 M3a 时就被合入 main，之后的 M3 余项 + M4 提交与 M5 的 PR #84 都只合到了分支 `feat/mobile-device-executor`，main 上并没有 M4 / M5——因此 M3 余项、M4、M5、M6 由分支 `feat/testing-m6-extensions` 一并汇总到 main（该分支已合入最新 main；fork 迁移因 main 的 913–915 让位而改号为 916 `test_run_parallelism`、917 `agent_runtime_test_host`、918 `autopilot_test_run`、919 `autopilot_run_test_run_index`，下一个 fork 前缀 920）；M1 在仓库 `coder-zkl1988/multica-device-mcp`。端到端验收状态不变：中枢已识别 5 台小米 + 1 台荣耀（adb 轨道），荣耀已装带执行器与 ADB 键盘的 APK（`ai.multica.mobile.dev`，API 指向 fork 的 staging），还差登录、配对、打开“测试机”、派发一条 `android_device` 用例。M6：iOS 经 PulsePhone 接入（中枢 e694df0）、`testcase recommend` 与推荐对话框、autopilot `test_run` 模式（迁移 918/919）。下一步：合并汇总 PR；真机验收（Android 经荣耀；iPhone 插到装了 PulsePhone 的测试机即可被中枢发现，本会话无 iPhone 可试）。
