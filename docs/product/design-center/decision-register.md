...
- 迁移清单（依次实施，完成即回填证据）：
  ...
  4. **演示模式与演讲者备注**。
  5. **持久分享与单文件发布** — `confirmed`，已完成（2026-08-22，服务端与公开页由并行会话 `design-center-three-tab-b2` 交付）。2026-08-22 移交该会话实施：它主要是服务端的路由、能力令牌与吊销模型，紧邻该会话已在做的删除/生命周期链路。产物复用本会话已完成的 `packages/views/designs/inline-prototype.ts`（`inlinePrototypePage` 把一页连同全部资源内联成自包含 HTML）。界线：**导出成单文件下载**属第 3 项（纯客户端），**发布到长期 URL**属本项（服务端）。
  6. **存为模板**。
  7. **重命名与改挂 Issue**。
  8. **预览/代码分栏**。
