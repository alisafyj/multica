...
> 2026-07-31 替代说明：用户已确认不再依据本方案自研一套 Open Design 云端等价流程。设计体系引擎与资产流程直接采用 Open Design，Multica 只增加 Project、Issue、Agent、云端存储与设计中心所需的薄适配层，见 `DC-037`。本文仅保留历史差距分析，不得作为目标数据模型或实施计划。
